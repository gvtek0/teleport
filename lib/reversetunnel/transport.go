...
func getConn(p *transportParams, servers []string, searchNames []string) (net.Conn, error) {
	...
	conn, err := tunnelDial(p.authClient, p.reverseTunnelServer, p.localClusterName, searchNames)
	if err != nil {
...
func tunnelDial(authClient auth.AccessPoint, tunnelServer Server, localClusterName string, searchNames []string) (net.Conn, error) {
